// client/src/provincial_admin/components/MunicipalitiesDashboard.tsx
...
type SortField = keyof MunicipalityMetrics;
type SortDirection = "asc" | "desc";

const MunicipalitiesDashboard: React.FC = () => {
  const [municipalityMetrics, setMunicipalityMetrics] = useState<MunicipalityMetrics[]>([]);
  ...
  const fetchMunicipalityMetrics = async () => {
    try {
      setLoading(true);
      const token = sessionStorage.getItem("token");
      const response = await axios.get(`${API_BASE_URL}/provincial-admin/municipality-metrics`, {
        params: { year: selectedYear, month: selectedMonth },
        headers: { Authorization: `Bearer ${token}` }
      });
      setMunicipalityMetrics(response.data);
      setError("");
    } catch (err: any) {
      console.error("Error fetching municipality metrics:", err);
      setError(err.response?.data?.error || "Failed to fetch municipality metrics");
    } finally {
      setLoading(false);
    }
  };
  ...
  const sortedData = [...municipalityMetrics].sort((a, b) => {
    ...
    return (
      <div style={{ 
        ...
        <p style={{ marginTop: "20px", color: "#666", fontSize: "1.1rem", fontWeight: 500 }}>
          Loading Municipalities Dashboard...
        </p>
      </div>
    );
  }
    ...
    return (
      <div style={{ 
        ...
        <button 
          ...
          onClick={fetchMunicipalityMetrics}
          style={{ 
            padding: "12px 24px", 
            borderRadius: "12px",
            fontWeight: 600,
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <div>
            <h3 style={{ color: "#37474F", margin: 0, fontWeight: 700, fontSize: "1.5rem" }}>
              ...
              Municipalities Performance Dashboard
            </h3>
            <p style={{ color: "#666", margin: "4px 0 0 0", fontSize: "0.95rem" }}>
              Overview of tourism metrics across all municipalities
            </p>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <span className="badge" style={{ 
              ...
              {sortedData.length} {sortedData.length === 1 ? 'Municipality' : 'Municipalities'}
            </span>
          </div>
        </div>
        ...
        {sortedData.length === 0 ? (
          <div style={{ 
            ...
            <p style={{ margin: 0 }}>
              No municipality metrics found for the selected period.
            </p>
          </div>
        ) : (
          ...
          <div style={{ 
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#00BCD4", fontWeight: 700, fontSize: "1.5rem" }}>
                {formatNumber(sortedData.reduce((sum, m) => sum + (m.total_check_ins || 0), 0))}
              </div>
              <div style={{ color: "#666", fontSize: "0.9rem" }}>Total Check-Ins</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#00BCD4", fontWeight: 700, fontSize: "1.5rem" }}>
                {formatNumber(sortedData.reduce((sum, m) => sum + (m.total_submissions || 0), 0))}
              </div>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#00BCD4", fontWeight: 700, fontSize: "1.5rem" }}>
                {formatPercentage(
                  sortedData.reduce((sum, m) => sum + (m.submission_rate || 0), 0) / 
                  sortedData.length
                )}
              </div>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ color: "#00BCD4", fontWeight: 700, fontSize: "1.5rem" }}>
                {formatNumber(sortedData.reduce((sum, m) => sum + (m.total_rooms || 0), 0))}
              </div>
...
export default MunicipalitiesDashboard;