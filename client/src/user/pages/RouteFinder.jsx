        ...
        setTimeout(() => {
          ...
          if (L && mapRef.current) {
            ...
            L.marker([location.latitude, location.longitude], {
              ...
              .addTo(map)
              .bindPopup(
                `<strong>📍 Your Location</strong><br/>${originName}<br/>Lat: ${location.latitude.toFixed(4)}<br/>Lng: ${location.longitude.toFixed(4)}`
              );
  ...
  useEffect(() => {
    if (route && destinationCoords && mapRef.current?.leafletMap) {
      ...
      L.marker(destinationCoords, {
        ...
        .addTo(map)
        .bindPopup(
          `<strong>🎯 Destination</strong><br/>${destination}<br/>Lat: ${destinationCoords[0].toFixed(4)}<br/>Lng: ${destinationCoords[1].toFixed(4)}`
        )
        .openPopup();
                ...
                {originName && (
                  <div className={`rounded-xl p-4 mb-6 border-2 ${
                    ...
                    <p className="text-sm font-semibold mb-2">
                      {isWithinPanglao ? (
                        <span className="text-green-800">📍 Within Panglao Municipality:</span>
                      ) : (
